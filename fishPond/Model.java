...
public class Model {
	...
	public boolean isSpaceAvailable(int r, int c) {
		...
		if(landscape[r][c] == ROCK)	//tests if location has a rock
			trueOrFalse = false;
		for(Plant p: plants){	//iterates for each plant and fish to see if the r and c is already occupied
	...
	public Model(Model other) {
		...
		ArrayList<Fish> test1 = new ArrayList<Fish>();
		ArrayList<Plant> test2 = new ArrayList<Plant>();
		for(Plant p: other.plants)
			test2.add(new Plant(p));
		for(Fish f: other.fish)
			test1.add(new Fish(f));
		fish = test1;
		plants = test2;
	}
	...
	public static void fishEatPlant(Fish f, Plant p) {
		if(p.getSize() < 75){	//when the size of a fish is smaller than a bite size
			f.eat(p.getSize());
	...
	public void removeDeadFish() {
		Iterator<Fish> iter = fish.iterator();
		while(iter.hasNext()){
			Fish f = iter.next();
			if(!(f.isAlive()))
				iter.remove();
	 ...
	 * then return false. */
	private boolean fishIsSurroundedByRocks(Fish f) { //tests all four spots adjacent to the fish
		if(landscape[f.getRow() + 1][f.getCol()] == ROCK && landscape[f.getRow() - 1][f.getCol()] == ROCK)
			if(landscape[f.getRow()][f.getCol() + 1] == ROCK && landscape[f.getRow()][f.getCol() - 1] == ROCK)
				return true;
			else
				return false;
		else
	...
	public void turnFish() {
		for(Fish f: fish){
			if(!(fishIsSurroundedByRocks(f))){
				int direction = f.getDirection();
				if(direction != 56 && direction != 57 && direction != 58 && direction != 59)//checks if fish has a valid direction
					throw new IllegalFishDirectionException(direction);
				else{
					int rowModifier = 0, colModifier = 0;//adds or subtracts based on direction of fish
					if(direction == 56)
						colModifier = -1;
					...
					if(landscape[f.getRow() + rowModifier][f.getCol() + colModifier] == ROCK)
						HittingRock = true;
					else
						HittingRock = false;
					
					while(HittingRock){	//goes into a loop that doesn't end until a rock is being hit
	...
	public void moveFish() {
		for(Fish f: fish){	//iterates every plant and every fish for every fish to see if there is a plant or fish there 
			if(!(fishIsSurroundedByRocks(f))){
				f.move();
				for(Plant p : plants){
					if(f.getCol() == p.getCol() && f.getRow() == p.getRow() && p.isAlive())
						fishEatPlant(f, p);
				}
				for(Fish fs: fish){
					if(fs.getCol() == f.getCol() && f.getRow() == fs.getRow() && !(f.equals(fs)))
						f.fight(fs);
				}
			}
		}
	...
	public void addPlant(Plant p) {
		if(!(landscape[p.getRow()][p.getCol()] == ROCK)){
			for(Plant pl: plants){
				if(pl.getRow() == p.getRow() && p.getCol() == pl.getCol() && !(p.equals(pl)))
						throw new IllegalPlantPositionException(0);
			}
			plants.add(p);
		}
		else
			throw new IllegalPlantPositionException(1);
	}
	...
	public void addFish(Fish f){
		if(!(landscape[f.getRow()][f.getCol()] == ROCK)){
			for(Fish fs: fish){
				if(fs.getRow() == f.getRow() && f.getCol() == fs.getCol() && !(f.equals(fs)))
						throw new IllegalFishPositionException(0);
			}
			fish.add(f);
		}
		else
			throw new IllegalFishPositionException(1);
	} 
	
	/** Returns a COPY of the fish list.  Hint:  Use the ArrayList<Fish> copy 
	 * constructor, or you will fail our tests! */
	public ArrayList<Fish> getFish() {
		ArrayList<Fish> test = new ArrayList<Fish>(fish);
		return test;
	}
	...
	public ArrayList<Plant> getPlants() {
		ArrayList<Plant> test = new ArrayList<Plant>(plants);
		return test;
	}
